...
**Error**: Existing `guide-state.md` is corrupted
...
- **Workaround**: Proceed without tests, add in Deploy phase
...
**Scenario**: `guide-state.md` is corrupted or inconsistent

**Recovery Steps**:
1. Create backup: `guide-state.md.backup`
...
3. Reset phase status in `guide-state.md`
...
3. Mark phase as "SKIPPED" in `guide-state.md`
...
- **Solution**: 
  ...
  2. Check if stage was marked complete in guide-state.md
...
**Error**: guide-state.md shows stage complete but artifacts don't exist
- **Cause**: State file updated but artifact generation failed
- **Solution**:
  1. Mark stage as incomplete in guide-state.md
...
- **Solution**:
  ...
  3. Correct guide-state.md to show single current stage
...
1. **Always validate state**: Check guide-state.md matches actual artifacts
